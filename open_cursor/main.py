...
class OpenCursorAgent:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            allowed_tools: List of tool names to enable. If None, empty list, or ["all"],
                          all tools are enabled. Available tools: read_file, write_file,
                          search_files, list_directory, execute_command, web_search,
    ...
    def _filter_tools_by_allowed(
        ...
        """Filter tool definitions based on allowed_tools list."""
        # If allowed_tools is None, empty list, or contains "all", return all tools
        if not self.allowed_tools or "all" in self.allowed_tools:
            return tools

        # Always include planning and control tools (create_plan, think, complete_task, subtask_done)
        control_tools = {
            "create_plan",
            "think",
            "complete_task",
    ...
    def _get_execution_tool_definitions(self) -> List[Dict]:
        ...
        return self._filter_tools_by_allowed(tools)
    ...
    def _get_execution_tools(self) -> Dict[str, Callable]:
        ...
        # If allowed_tools is None, empty list, or contains "all", return all tools
        if not self.allowed_tools or "all" in self.allowed_tools:
            return all_exec_tools

        # Always include control tools
        control_tools = {"subtask_done"}

        # Filter tools based on allowed list
        filtered_tools = {}
        for tool_name, tool_func in all_exec_tools.items():
            if (
                tool_name in control_tools
                or tool_name in self.allowed_tools
            ):
        ...
        try:
            ...
            if isinstance(response, list) and len(response) > 0:
                ...
                if (
                    ...
                    logger.info(
                        f"Plan created successfully: {result}"
                    )
                    ...
                    logger.error(
                        "Expected create_plan tool call in planning phase"
                    )
                    self.context.current_state = AgentState.ERROR
            elif response:
            ...
            if isinstance(response, list) and len(response) > 0:
                ...
                if (
                    ...
                    if function_name in self.tools:
                        ...
                        if (
                            next_task
                            and function_name != "subtask_done"
                        ):
        ...
        try:
            ...
            if isinstance(response, list) and len(response) > 0:
                ...
                if (
                    ...
                    logger.info(
                        f"Executing thinking tool: {function_name}"
                    )
                    ...
                    if function_name in self.tools:
                        ...
                        logger.error(
                            f"Unknown thinking tool: {function_name}"
                        )
                        self.context.current_state = AgentState.ERROR
                        return
            elif response:
